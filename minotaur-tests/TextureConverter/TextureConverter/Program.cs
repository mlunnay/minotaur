...
namespace TextureConverter
{
  class Program
  {
    ...
    static TextureType _textureType = TextureType.Texture2D;
    ...
    private static bool ParseCommandLine(string[] args)
    {
      ...
      {
        {"t=|type=", "The Texture Type that is being input. One of '1D', '2D', 'Cube'. Defaults to 2D.", a => SetTextureType(a)},
        {"o=|output=", "The output file to write the texture to.", a => _outputName = a},
        {"a|nopremultiply", "Do not premultiply the images alpha.", a => _opaqueData["PremultiplyAlpha"] = false},
        {"m|mipmaps", "Generate Mipmaps for the texture.", a => _opaqueData["GenerateMipmaps"] = true},
      ...
      if (extra.Count != 1)
      {
        Console.WriteLine("Error: Image file was not given.");
        ShowHelp(p);
        return false;
      }
    ...
    {
      Console.WriteLine("Usage: TextureConverter [OPTIONS] IMAGE");
      Console.WriteLine("Converts an image to a Minotaur Texture file.");
      Console.WriteLine();
      Console.WriteLine("Options:");
      options.WriteOptionDescriptions(Console.Out);
    }

    private static void SetTextureType(string value)
    {
      if (value.Equals("1D", StringComparison.InvariantCultureIgnoreCase))
        _opaqueData["TextureType"] = TextureType.Texture1D;
      else if (value.Equals("2D", StringComparison.InvariantCultureIgnoreCase))
        _opaqueData["TextureType"] = TextureType.Texture2D;
      else if (value.Equals("Cube", StringComparison.InvariantCultureIgnoreCase))
        _opaqueData["TextureType"] = TextureType.TextureCube;
    }
  }
}
